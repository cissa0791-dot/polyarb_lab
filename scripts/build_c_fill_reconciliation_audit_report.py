...
from src.live.c_fill_reconciliation_audit import (  # noqa: E402
    READY_STATUS,
...
DEFAULT_OUT = DEFAULT_REPORTS_DIR / "c_fill_reconciliation_audit_latest.json"
...
REPORT_FILES = {
    ...
    "token": "single_side_probe_authorization_token_latest.json",
    "gate": "live_readiness_gate_latest.json",
}


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build C-class fill reconciliation audit report.")
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    report = build_c_fill_reconciliation_audit(
        probe=loaded["probe"],
    ...
    attach_writer_metadata(
        ...
        source_files=[Path(__file__), ROOT / "src" / "live" / "c_fill_reconciliation_audit.py"],
        root=ROOT,
    )
    return report
