...
def main(argv: list[str] | None = None) -> int:
    ...
    params, blockers = _resolve_params(args, planner)
    if blockers:
...
def _resolve_params(args: argparse.Namespace, planner: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    blockers: list[str] = []
    ...
    params = {
        "market_slug": plan.get("market_slug") if plan else args.market_slug,
        "max_live_risk_usdc": planner.get("max_live_risk_usdc") if planner else args.max_live_risk_usdc,
        "quote_price": plan.get("quote_price") if plan else args.quote_price,
        "quote_size": plan.get("quote_size") if plan else args.quote_size,
        "hold_seconds": planner.get("hold_seconds") if planner else args.hold_seconds,
        "planner_hash": planner.get("planner_hash") if planner else None,
        "planner_snapshot_ts": planner.get("planner_snapshot_ts") if planner else None,
        "planner_expires_at": planner.get("planner_expires_at") if planner else None,
    }
    ...
    for key, explicit in explicit_checks.items():
        if not planner or explicit in {None, ""}:
            continue
