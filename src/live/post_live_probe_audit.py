...
def build_post_live_probe_audit(
    *,
    ...
    lineage_guard = _build_lineage_guard(
        probe=probe,
    ...
    return {
        ...
        "final_safety": final_safety,
        "lineage_guard": lineage_guard,
        "lineage_status": lineage_guard["status"],
        "lineage_blockers": lineage_guard["blockers"],
        "source_report_timestamps": lineage_guard["source_report_timestamps"],
        "matched_order_id": lineage_guard["matched_order_id"],
        "matched_market_slug": lineage_guard.get("matched_market_slug") or lineage_guard.get("expected_market_slug"),
        "tri_party_consistency": tri_party_consistency,
        "pnl_accounting": {
...
def _blockers(
    *,
    ...
    if lineage_guard["status"] != "POST_PROBE_REPORT_LINEAGE_READY":
        blockers.extend(lineage_guard["blockers"])
    return _unique(blockers)


def _build_lineage_guard(
    *,
    probe: dict[str, Any],
    ...
    return {
        "status": "POST_PROBE_REPORT_LINEAGE_READY" if not blockers else "POST_PROBE_REPORT_LINEAGE_BLOCKED",
        "blockers": blockers,
...
def _market_slug_from_planner(planner: dict[str, Any]) -> str | None:
    ...
    return _text(plan.get("market_slug") or planner.get("target_market_slug") or planner.get("market_slug"))
